<template>
    <BreezeFormSection>
        <template #title>
            {{ $t('pages.forms.fields.options.list.title') }}
        </template>

        <template #description>
            {{ $t('pages.forms.fields.options.list.description') }}
        </template>

        <template #form>
            <div class="col-span-6">
                <div class="max-w-xl text-sm text-gray-600">
                    {{ $t('pages.forms.fields.options.list.form.listInfo') }}
                </div>
            </div>
                
            <div class="relative z-0 mt-1 border border-gray-200 rounded-lg cursor-pointer col-span-6 sm:col-span-4">
                <button class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200" type="button" v-for="(option, i) in field.options" :key="option.id" :class="{'border-t border-gray-200 rounded-t-none': i > 0, 'rounded-b-none': i != Object.keys(field.options).length - 1}">
                    ...
                    <div class="w-full">
                        <div class="flex items-center justify-between">
                            ...
                            <Link 
                                ...
                                :href="route('form-field-option.show', {'id': option.id})">
        ...
        <template #actions>
            <Link class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray transition ease-in-out duration-150" :href="route('form-field-option.create', {'field': field.id})">
                {{ $t('pages.forms.fields.options.list.actions.createButton') }}
            </Link>
        </template>
    </BreezeFormSection>
</template>
