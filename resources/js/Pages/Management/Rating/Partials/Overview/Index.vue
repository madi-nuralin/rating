                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg dark:bg-gray-800">
                    <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-800">
                        <div class="grid gap-6 grid-cols-8 pb-6">
                            <form class="col-span-8 sm:col-span-6" @submit.prevent="$emit('submitted')">
                                <div class="grid gap-6 grid-cols-6 pb-1">
                                    <BreezeInput type="text"
                                                 id="email"
                                                 class="col-span-6 block w-full"
                        ...
                        <div class="-mx-6 overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                                ...
                                <tr class="flex flex-col md:table-row">
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-100">
                                        {{ translate[0]('table.thead.user') }}
                                    </th>
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-100">
                                        {{ translate[0]('table.thead.position') }}
                                    </th>
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-100">
                                        Статус утверждения
                                    ...
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-100">
                                        {{ translate[0]('table.thead.progress') }}
                                    </th>
                                    <th scope="col"
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-100">
                                        Jтчет
                                ...
                                <tr class="flex flex-col md:table-row" v-for="user in $page.props.rating.users.data"
                                    v-if="$page.props.rating && $page.props.rating.users">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div v-for="employement in user.employements"
                                             v-if="Object.keys(user.employements).length > 0">
                                            <div class="text-sm text-gray-900 dark:text-gray-100 truncate">
                                                {{ employement.position.name }}
                                            </div>
                                            <div class="text-sm text-gray-500 truncate">{{
                                                    employement.department.name
                                                }}
                                            </div>
                                        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex justify-start">
                                            <breeze-badge :color="user.is_approved ? 'green' : 'yellow'">
                                                {{
                                                    $t(`pages.dashboard.partials.showApprovement.status[${user.is_approved ?
                                                        1 : 0}]`)
                                                }}
                                            </breeze-badge>
                                        </div>
                                        ...
                                        <div class="flex flex-col space-y-2">
                                            <div class="flex items-center space-x-2" v-for="verifier in user.verifiers">
                                                <div style="min-width: 40px;">
                                                    <breeze-progress :color="'indigo'" :rounded="false"
                                                                     :percentage="verifier.statistics.completed/verifier.statistics.total * 100"/>
                                                </div>
                                                <p class="text-sm text-gray-500">{{ verifier.statistics.completed }} /
                                                    {{ verifier.statistics.total }}</p>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a :href="route('rating.overview.downloadReport', {'rating': $page.props.rating.id, 'user': user.id})"
                                           class="text-gray-400 hover:text-blue-500 underline hover:scale-110" download>Загрузить
                                            отчет</a>
                                    </td>
                            ...
                            <div class="flex justify-center items-center my-2">
                                <breeze-pagination :links="$page.props.rating.users.links"
                                                   v-if="$page.props.rating && $page.props.rating.users"/>
                            </div>
                            <!-- /Table -->
                        </div>
...
export default {
    ...
    methods: {
        submit(event) {
            this.form.get(route('rating.overview.index', {
                'id': this.$page.props.rating.id,
                'email': this.form.verifier,
