<template>
    <BreezeFormSection>
        <template #title>
            {{ $t('pages.management.rating.list.title') }}
        </template>

        <template #description>
            {{ $t('pages.management.rating.list.description') }}
        </template>
                ...
                <BreezeList :items="ratings">
                    <template #item="{item}">
                        <div class="flex justify-between">
                            ...
                            <Link class="ml-2 text-sm text-gray-400 underline" :href="route('rating.show', {'id': item.id})">
