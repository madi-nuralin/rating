<template>
    <breeze-action-section>
        <template #title>
            {{ $t('pages.management.department.delete.title') }}
        </template>

        <template #description>
            {{ $t('pages.management.department.delete.description') }}
        </template>

        <template #content>
            <div class="max-w-xl text-sm text-gray-600">
                {{ $t('pages.management.department.delete.content.deleteInfo') }}
            </div>

            <div class="mt-5">
                <breeze-button-danger @click="confirmDepartmentDeletion">
                    {{ $t('pages.management.department.delete.content.deleteButton') }}
            ...
            <breeze-modal-confirmation :show="confirmingDepartmentDeletion" @close="confirmingDepartmentDeletion = false">
                <template #title>
                    {{ $t('pages.management.department.delete.content.modal.title') }}
                </template>

                <template #content>
                    {{ $t('pages.management.department.delete.content.modal.content') }}
                </template>

                <template #footer>
                    <breeze-button-secondary @click="confirmingDepartmentDeletion = false">
                        {{ $t('pages.management.department.delete.content.modal.footer.cancelButton') }}
                    ...
                    <breeze-button-danger class="ml-2" @click="deleteDepartment" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                        {{ $t('pages.management.department.delete.content.modal.footer.deleteButton') }}
                    </breeze-button-danger>
                </template>
